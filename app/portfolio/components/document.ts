...
function bootCustomCursor(){if(window.matchMedia("(pointer: coarse)").matches)return;const cursor=document.createElement("div");cursor.className="vf-cursor";document.body.appendChild(cursor);document.body.classList.add("has-custom-cursor");let tx=window.innerWidth/2,ty=window.innerHeight/2,cx=tx,cy=ty,visible=false,typing=false;function syncVisibility(){cursor.classList.toggle("vf-cursor-visible",visible&&!typing);cursor.classList.toggle("vf-cursor-hidden",typing)}function isTextTarget(target){return target instanceof Element&&Boolean(target.closest("input,textarea,[contenteditable='true']"))}function tick(){cx+=(tx-cx)*.18;cy+=(ty-cy)*.18;cursor.style.transform="translate3d("+cx+"px,"+cy+"px,0) translate(-50%,-50%)";requestAnimationFrame(tick)}window.addEventListener("pointermove",(event)=>{tx=event.clientX;ty=event.clientY;typing=isTextTarget(event.target);if(!visible)visible=true;syncVisibility()});window.addEventListener("pointerleave",()=>{visible=false;syncVisibility()});document.addEventListener("focusin",(event)=>{typing=isTextTarget(event.target);syncVisibility()});document.addEventListener("focusout",()=>{typing=false;syncVisibility()});tick();}
...
<div class="portfolio-shell">
  ...
  <main class="site-main">
    <section id="home" class="orbit-stage panel reveal" data-reveal>
      ...
      <div class="orbit-bg-character" aria-hidden="true">${escapeHtml(initials)}</div>
